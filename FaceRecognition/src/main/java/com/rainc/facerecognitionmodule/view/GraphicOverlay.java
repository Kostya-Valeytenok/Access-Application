...
/**
 * A view which renders a series of custom graphics to be overlayed on top of an associated preview (i.e., the camera preview). The creator
 * can add graphics objects, update the objects, and remove them, triggering the appropriate drawing and invalidation within the view.
 *
 * <p>Supports scaling and mirroring of the graphics relative the camera's preview properties. The
 * idea is that detection items are expressed in terms of an image size, but need to be scaled up to the full view size, and also mirrored
 * in the case of the front-facing camera.
 *
...
public class GraphicOverlay extends View
{
  ...
  private int naturalRotation = Surface.ROTATION_0;
   ...
   * Graphic#draw(Canvas)} method to define the graphics element. Add instances to the overlay using {@link GraphicOverlay#add(Graphic)}.
   */
  public abstract static class Graphic
  {
    ...
    /**
     * Draw the graphic on the supplied canvas. Drawing should use the following methods to convert to view coordinates for the graphics
     * that are drawn:
     *
     * <ol>
  ...
  public void setImageSourceInfo(int imageWidth, int imageHeight, boolean isFlipped, int naturalRotation, int currentRotation)
  {
  ...
  protected void onDraw(Canvas canvas)
  {
    ...
    canvas.save();
    canvas.rotate((currentRotation - naturalRotation) * 90f, getWidth() / 2f, getHeight() / 2f);
